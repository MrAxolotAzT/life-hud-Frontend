        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div className="card" style={{ padding: 18, borderTop: "3px solid #7C3AED" }}>
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 12 }}>
              ...
              <div style={{ textAlign: "right" }}>
                <div style={{ fontFamily: "'Orbitron',monospace", fontSize: 22, fontWeight: 900, color: "#7C3AED" }}>
                  {Math.round((f.balance / 40000) * 100)}%
                </div>
              </div>
            </div>
            <ProgressBar value={f.balance} max={40000} color="#7C3AED" height={8} />
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginTop: 8 }}>
              <span style={{ color: "#10B981", fontWeight: 700 }}>${f.balance.toLocaleString()} ahorrados</span>
            ...
            <div style={{ marginTop: 10, padding: "9px 12px", borderRadius: 8, background: "rgba(6,182,212,0.07)", border: "1px solid rgba(6,182,212,0.2)", fontSize: 11, color: "#67E8F9", lineHeight: 1.6 }}>
              🤖 A este ritmo lo logras en <strong>{Math.ceil((40000 - f.balance) / ahorro)} meses</strong>. Con $1k extra/mes llegarías en {Math.ceil((40000 - f.balance) / (ahorro + 1000))} meses.
            </div>
          </div>
...
const ObjetivosPage = () => {
  const [goals, setGoals] = useState([
    {
      id: 1, key: "auto", title: "Mi Primer Auto", customText: "Quiero un auto propio para tener movilidad independiente",
      progress: 45, status: "active", created: "Ene 2026",
      hitos: [
        { id: 1, titulo: "Juntar primer $10,000", fecha: "2026-02-28", done: true,  emoji: "💰" },
        { id: 2, titulo: "Definir modelo y presupuesto", fecha: "2026-03-15", done: true,  emoji: "🔍" },
        { id: 3, titulo: "Juntar $30,000 (enganche)", fecha: "2026-05-31", done: false, emoji: "🏦" },
        { id: 4, titulo: "Tramitar crédito automotriz", fecha: "2026-06-15", done: false, emoji: "📄" },
        { id: 5, titulo: "Comprar el auto", fecha: "2026-07-01", done: false, emoji: "🚗" },
      ],
      sprint: {
        semana: "10-16 Mar",
        tareas: [
          { id: 1, texto: "Comparar precios en 3 agencias", done: false, modulo: "💰 Finanzas" },
          { id: 2, texto: "Apartar $3,000 extra al ahorro", done: true,  modulo: "💰 Finanzas" },
          { id: 3, texto: "Investigar créditos automotrices", done: false, modulo: "📋 Tareas" },
          { id: 4, texto: "Calcular costo de mantenimiento mensual", done: false, modulo: "💰 Finanzas" },
        ],
      },
    },
    {
      id: 2, key: "empleo", title: "Trabajo Remoto Python", customText: "Conseguir empleo como Python Developer remoto pagado en USD",
      progress: 20, status: "active", created: "Feb 2026",
      hitos: [
        { id: 1, titulo: "Completar FastAPI (100%)", fecha: "2026-04-01", done: false, emoji: "⚡" },
        { id: 2, titulo: "Subir 3 proyectos a GitHub", fecha: "2026-04-15", done: false, emoji: "💻" },
        { id: 3, titulo: "Aplicar a 20 empleos", fecha: "2026-05-01", done: false, emoji: "📨" },
        { id: 4, titulo: "Primera entrevista técnica", fecha: "2026-05-15", done: false, emoji: "🎙️" },
        { id: 5, titulo: "Oferta de trabajo aceptada", fecha: "2026-06-30", done: false, emoji: "🏆" },
      ],
      sprint: {
        semana: "10-16 Mar",
        tareas: [
          { id: 1, texto: "Terminar módulo de autenticación FastAPI", done: false, modulo: "📚 Learning" },
          { id: 2, texto: "Hacer 3 ejercicios LeetCode (easy)", done: true,  modulo: "📚 Learning" },
          { id: 3, texto: "Actualizar README del proyecto principal", done: false, modulo: "📋 Tareas" },
          { id: 4, texto: "Estudiar 1h extra SQL avanzado", done: false, modulo: "📚 Learning" },
        ],
      },
    },
  ]);
  ...
  const roadmap = buildAIRoadmap(activeGoal.key, mockData);
  const datosReales = getDatosReales(activeGoal.key, mockData);
  ...
  const addGoal = () => {
    ...
    setTimeout(() => {
      const nuevo = {
        id: Date.now(), key: form.category, title: form.title, customText: form.text,
        progress: 0, status: "active", created: "Mar 2026",
        hitos: [], sprint: { semana: "Esta semana", tareas: [] },
      };
      setGoals(p => [...p, nuevo]);
  ...
  const hitosCompletados = activeGoal.hitos.filter(h => h.done).length;
  const sprintDone = activeGoal.sprint.tareas.filter(t => t.done).length;
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", gap: 5, marginTop: 3, flexWrap: "wrap" }}>
                      ...
                      <span className="tag" style={{ background: `${statusConfig[goal.status].color}15`, color: statusConfig[goal.status].color }}>{statusConfig[goal.status].label}</span>
                    </div>
                  </div>
                </div>
...
const FinanzasPage = () => {
  ...
  const addTxn = () => {
    ...
    const newTxn = {
      ...
      type: form.type, category: form.category, date: "Ahora",
  ...
  const proyeccion = Array.from({ length: proyMeses }, (_, i) => ({
    mes: i + 1,
    balance: Math.round(f.balance + ahorro * (i + 1)),
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
          <div className="card" style={{ padding: 18 }}>
            ...
            {f.monthlyHistory.map((m, i) => {
              const prev = f.monthlyHistory[i - 1];
            ...
            <div className="card" style={{ padding: 20 }}>
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: 10, height: 180, marginBottom: 12 }}>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                  <div style={{ fontSize: 9, color: "#A78BFA", fontWeight: 700 }}>${(f.balance/1000).toFixed(1)}k</div>
                  <div style={{ width: "100%", height: `${(f.balance / maxProy) * 160}px`, background: "#4C1D95", borderRadius: "4px 4px 0 0", minHeight: 4 }} />
              ...
              <div style={{ padding: "12px 14px", borderRadius: 10, background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.2)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                  ...
                  <span style={{ color: "#F1F5F9", fontWeight: 700, fontFamily: "'Orbitron',monospace" }}>${f.balance.toLocaleString()}</span>
                </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div className="card" style={{ padding: 18 }}>
                ...
                {[
                  ...
                  const mesesNecesarios = Math.ceil((m.meta - f.balance) / ahorro);
                ...
                {[
                  ...
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "9px 0", borderBottom: "1px solid #1A1A28", fontSize: 12 }}>
                    ...
                    <span style={{ color: e.color, fontWeight: 700, fontFamily: "'Orbitron',monospace" }}>
                      ${Math.round(f.balance + e.ahorro * proyMeses).toLocaleString()}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div className="card" style={{ padding: 18 }}>
            ...
            <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
              <svg width="120" height="120" viewBox="0 0 120 120">
                ...
                <text x="60" y="64" textAnchor="middle" fill="#64748B" fontSize="9" fontFamily="Rajdhani,sans-serif">${(f.monthExpense / 1000).toFixed(1)}k</text>
              </svg>
              <div style={{ flex: 1 }}>
                {f.expenseByCategory.map((cat, i) => (
...
const FitnessPage = () => {
  ...
  const finalizarEntrenamiento = ({ duracion, series }) => {
    const nuevo = {
      id: Date.now(),
      fecha: "Ahora",
      tipo: tipoActivo,
  ...
  return (
    <div className="page" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
        {[
          { label: "Racha actual", value: `${mockData.fitness.streak}d`, icon: "🔥", color: "#EF4444" },
